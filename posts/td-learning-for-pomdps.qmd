...
format:
  html:
    css: styles.css   # we'll add a tiny style below
---

> **TL;DR.** A classic result by [@singh1994learning] shows that memoryless TD learning fails in POMDPs. One fix is to use multi-step TD learning with an internal (memory) state to mitigate perceptual aliasing. This idea is studied in this post in a simple Hidden Markov Model setting with examples.

![](POMDP.png){fig-alt="Partial observability scene" width="50%" column="page" .banner-img}
...
In order to compute a low-variance estimate of the (natural) policy gradient for this linear-softmax FSC within an actor-critic framework, we need to evaluate $\mathcal{Q}^{\pi_\theta}(A_k,Y_k,Z_k^c)$ under this internal state representation. This blog post is about this part, the critic.
...
**Example 1 (When TD(0) fails to learn $V_s^\star$).** Consder an HMRP with $\mathcal S = \mathcal Y = \{0,1\}$, and $r(s) = s$. Let $P(0|1) = q$ and $P(1|0) = p$ for some $p, q \in (0,1)$, and $\gamma \in (0, 1)$ be the discount factor. As TD(0) cannot distinguish different latent states due to the noisy observation and the lack of memory, it converges to $$v^\star = \frac{1}{1-\gamma}\cdot \frac{p}{q+p} = V_0^\star + \frac{p}{p+q}(V_1^\star-V_0^\star).$$

For the true value function with the initial latent distribution $S_0$, we have $V_{S_0}^\star = V_0^\star + (V_1^\star-V_0^\star)\mathbf{1}_{\{S_0=1\}}$. Therefore, $$\mathbb{E}[(V_{S_0}^\star-v^\star)^2] = (V_1^\star-V_0^\star)^2\mathbb{E}[(\frac{p}{p+q}-\mathbf{1}_{\{S_0=1\}})^2]=Var(\mathbf{1}_{\{S_0=1\}})(V_1^\star-V_0^\star)^2,$$ where $Var(S_0)$ is under the stationary distribution of $S_k$. As such, TD(0) suffers from a non-vanishing mean-square error unless the stationary distribution of $S_k$ is a Dirac measure.

For a given internal state $z_0$, the value function is $$V(z_0) = V_0^\star + (V_1^\star-V_0^\star)P(S_0=1|Z_0=z_0) = \frac{\gamma p + (1-\gamma)P(S_0=1|Z_0=z_0)}{(1-\gamma)\Big[\gamma(p+q)+1-\gamma\Big]}.$$ Except the highly degenerate situation where $Z_0$ is independent from $S_0$, this indicates that $V(z_0)\neq v^\star$.

Thus, except the degenerate cases of (i) $S_k\underset{k\rightarrow \infty}{\Rightarrow}\bar{S}$ where $\bar{S}$ is deterministic, and (ii) uncorrelated $Z_0$ and $S_0$, TD(0) fails even in this simple problem.

**Example 2 (When TD(0) fails to learn $V(z_0)$).** Consider the two-state HMRP with $\mathcal S = \{0,1\}$, $r(s) = s$ and $\Phi(1|s) = c_s\in(0,1)$. Let $P(0|1) = P(1|0) = 0.01$, and $\gamma = 0.99$. Consider an internal state $Z_k = (Y_{k-1}, Y_k)$. We want to compute $V(z_0) = \mathbb{E}[\sum_k \gamma^k r(S_k)|Z_0=z_0]$. For this purpose, one can use tabular-TD(0) with the surrogate state $Z_k=(Y_{k-1},Y_k)$. In this case, the limit point of TD(0) would be the unique fixed point of $$(T_1v)(z_0):=P(S_0=1|Z_0=z_0) + \gamma \sum_{z_1\in\{0,1\}^2}v(z_1)P(Z_1=z_1|Z_0=z_0).$$ Now, let's diagnose why the true value is **not** the fixed point of the Bellman equation. Note that we can write 
\begin{align*}
