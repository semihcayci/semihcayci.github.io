...
The analysis can be taken one step further indeed. The argument below can be shifted in time, so let us consider $t=0$ for notational simplicity. Let $\mathcal{F}_n :=\sigma(Z_0)=\sigma(Y_{-n+1},\ldots,Y_0)$ be the $\sigma$-algebra generated by the internal state, and $b^{(n)} := P(S_0 = 1|\mathcal{F}_n)$. Then, $V(z_0) =  \mathbb E[\sum_{t=0}^\infty\gamma^tr(S_t)|\mathcal{F}_n]$. Since $V_s^\star = V_0^\star + (V_1^\star-V_0^\star)\mathbf{1}_{\{S_0=s\}}$ and $V(z_0) = V_0^\star + (V_1^\star-V_0^\star)b^{(n)}$, we obtain $$\mathbb{E}[(V_{S_0}^\star-V(z_0))^2] = (V_1^\star-V_0^\star)^2\,\mathbb E\!\left[(\mathbf{1}_{\{S_0=1\}}-b^{(n)})^2\right].$$ Furthermore, since $b^{(m)}=\mathbb E[\mathbf{1}_{\{S_0=1\}}|\mathcal F_n]$, we have 
