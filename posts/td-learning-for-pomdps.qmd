...
**Example 2 (When TD(0) fails to learn $V(z_0)$).** Consider the two-state HMRP with $\mathcal S = \{0,1\}$, $r(s) = s$ and $\Phi(1|s) = c_s\in(0,1)$ now. Let $P(0|1) = P(1|0) = 1/2$, and $\gamma = 0.9$. Consider an internal state $Z_k = (Y_{k-1}, Y_k)$. We want to compute $V(z_0) = \mathbb{E}[\sum_k \gamma^k r(S_k)|Z_0=z_0]$. For this purpose, one can use TD(0) with $f(z_0;w)=\langle w, \psi(z_0)\rangle$ for a feature map $\psi:\mathcal{Y}^2\rightarrow \mathbb{R}^d$. 

In the proof of Theorem 4.1 in [@cayci2024nac], we characterize the issues related to the fixed point of single-step TD(0) for general POMDPs.
