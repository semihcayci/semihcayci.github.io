...
The analysis of $m$-step TD(0) with an internal state $Z_k=(Y_{k-n},\ldots,Y_k)$ can be taken one step further indeed. Let us consider Example 1 with a general $\{\Phi(y|s):s\in\mathcal S,y\in\mathcal{Y}\}$. Let $\mathcal{F}_n :=\sigma(Z_0)=\sigma(Y_{-n+1},\ldots,Y_0)$ be the $\sigma$-algebra generated by the internal state, and $b^{(n)} := P(S_0 = 1|\mathcal{F}_n)$. Then, $V_n(z_0) =  \mathbb E[\sum_{k=0}^\infty\gamma^kr(S_k)|\mathcal{F}_n]$. Since $V_s^\star = V_0^\star + (V_1^\star-V_0^\star)\mathbf{1}_{\{S_0=s\}}$ and $V_n(z_0) = V_0^\star + (V_1^\star-V_0^\star)b^{(n)}$, we obtain $$\mathbb{E}[(V_{S_0}^\star-V_n(Z_0))^2] = (V_1^\star-V_0^\star)^2\,\mathbb E\!\left[(\mathbf{1}_{\{S_0=1\}}-b^{(n)})^2\right].$$ Furthermore, since $b^{(n)}=\mathbb E[\mathbf{1}_{\{S_0=1\}}|\mathcal F_n]$, we have 
