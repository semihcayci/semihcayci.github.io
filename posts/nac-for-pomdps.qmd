...
It is very easy to construct an example which shows that (memoryless) TD learning cannot learn the value functions in a POMDP, which is known as the perceptual aliasing pathology [@singh1994learning]. Let us focus on learning $$V(z_0):=\mathbb E[\sum_{k=0}^\infty \gamma^k r(S_k)|Z_0=z_0]$$ for an internal state $Z_t$ (e.g., $Z_t = (Y_{t-m+1},\ldots,Y_t)$), where $(S_k, r(S_k), Y_k)$ is a hidden Markov reward process.

**Example 1** Consider a (hidden) Markov reward process with $\mathcal S = \{0,1\}$, $r(s) = s$ and $\Phi(1|s) = 1$ for all $s\in\mathcal{S}$. Let $P(0|1) = p$ and $P(1|0) = q$ for some $p, q \in (0,1)$, and $\gamma \in (0, 1)$ be the discount factor. Then, TD(0) converges to $v^\star = \frac{1}{1-\gamma}\cdot \frac{p}{q+p}$. On the other hand, the value function for a given internal state $Z_0=z_0$ is $$V(z_0) = \frac{\gamma p + (1-\gamma)P(S_0=1|Z_0=z_0)}{(1-\gamma)\Big[\gamma(p+q)+1-\gamma\Big]}.$$ As such, memoryless TD learning cannot learn $V(z_0)$.
...
To see how $m$-step TD learning with internal state $Z_t = (Y_{t-m+1},\ldots,Y_t)$ mitigates perceptual aliasing, let us focus on the limit point of the algorithm. To that end, let $$\bar{r}_m(z) := \mathbb E[\sum_{k=0}^{m-1}r(S_k)|Z_0=z]$$, $(K_mv)(z):=\mathbb{E}[v(Z_m)|Z_0=z]$, and $$(T_mv)(z) = \bar{r}_m(z) + \gamma^m(K_mv)(z).$$ Then, it is straightforward to show that $T_m$ is a contractive operator with modulus $\gamma^m$, and admits a unique fixed point $$V(z) = \mathbb E[\sum_{k=0}^\infty r(S_k)|Z_0=z].$$ Alternatively, one can write $$V = \sum_{k=0}^\infty (\gamma^mK_m)^k\bar{r}_m$$ for any $m\in\mathbb N$. Let $\xi$ be the invariant distribution of $\{Z_k:k\in\mathbb N\}$: $\xi(z_t)=\sum_{s_{t-m+1:t}}\prod_{k=0}^{m-1}\Phi(y_{t-k}|s_{t-k})\mu(s_{t-k})$, where $\mu$ is the stationary distribution of $\{S_k:k\in\mathbb N\}$. Assume that the Gram matrix $\mathbb{E}_{Z_t\sim \xi}[\psi(Z_t)\otimes\psi(Z_t)]\succ 0$. Let $\pi$ be the ortogonal projection onto $\mathcal V := \{z\mapsto \langle \psi(z),w\rangle: w \in \mathbb R^d$ with respect to $L^2(\xi)$. Then, $m$-step TD learning with the internal state $Z_t$ converges to the unique fixed point $v_m$ of the equation $$v = \pi\{\bar{r}_m+\gamma^m K_m v\}.$$ Using this, we can show that $$\|v_m - \pi V\|_{L^2(\xi)} \leq \frac{\gamma^m}{1+\gamma^m}\|V-\pi V\|_{L^2(\xi)}.$$ As such, it is possible to learn $V(z_0)$ via $m$-step TD learning with an additional error term $\mathcal{O}(\gamma^m)$.
