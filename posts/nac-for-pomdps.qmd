...
date: 2025-10-12
description: "Finite-time guarantees for natural actor–critic with m-step TD and sliding-window controllers."
...
## Formal POMDP model (and why it’s hard)
...
- transition kernel $P(\mathrm d s' \mid s,a)$,
- observation kernel $O(\mathrm d y \mid s')$ (can depend on $a$ in general),
...
**Why challenging?**  
Partial observability means the optimal $A_k$ depends on the **entire history** $H_k=(Y_{0:k},A_{0:k-1})$ via the Bayes belief $b_k(s)=\Pr(S_k=s\mid H_k)$. Working exactly in belief space yields a continuous, often high-dimensional MDP; with function approximation, **perceptual aliasing** (same $Y_k$, different $S_k$) biases TD targets. The paper sidesteps this by learning **finite-memory controllers** and using **$m$-step TD** to reduce aliasing.
