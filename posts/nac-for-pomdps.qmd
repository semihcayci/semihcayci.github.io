...
The analysis can be taken one step further indeed. Let $\mathcal{F}_m :=\sigma(Z_0)=\sigma(Y_{-m+1},\ldots,Y_0)$ be the $\sigma$-algebra generated by the internal state, and $b^{(m)} := P(S_0 = 1|\mathcal{F}_m)$. Then, $V(z_0) =  \mathbb E[\sum_{t=0}^\infty\gamma^tr(S_t)|\mathcal{F}_m]$. Since $V_s^\star = V_0^\star + (V_1^\star-V_0^\star)\mathbf{1}_{\{s=1\}}$ and $V(z_0) = V_0^\star + (V_1^\star-V_0^\star)b^{(m)}$, we obtain $$\mathbb{E}[(V_{s_0}^\star-V(z_0))^2] = (V_1^\star-V_0^\star)^2\,\mathbb E\!\left[(\mathbf{1}_{\{s_0=1\}}-b^{(m)})^2\right].$$ Furthermore, since $b^{(m)}=\mathbb E[\mathbf{1}_{\{s_0=1\}}|\mathcal F_m]$, we have $E\!\left[(\mathbf{1}_{\{s_0=1\}}-b^{(m)})^2\right]=\mathbb E[Var(\mathbf{1}_{\{s_0=1\}}|\mathcal F_m)]\downarrow \mathbb E[Var(\mathbf{1}_{\{s_0=1\}}|\mathcal F_\infty)]$ as $m\rightarrow\infty$ due to the law of total variance for nested $\sigma$-algebras. Therefore, $$\mathbb{E}[(V_{s_0}^\star-V(z_0))^2] \downarrow (V_1^\star-V_0^\star)^2\,\mathbb E[Var(\mathbf{1}_{\{s_0=1\}}|\mathcal F_\infty)].$$ In case $S_0$ is measurable with respect to $\sigma(Y_{-\infty},\ldots,Y_0$, this implies the learnability of $V_s^\star$ via TD learning with a sliding-window controller.
