...
The analysis can be taken one step further. Let $\mathcal{F}_m :=\sigma(Z_0)=\sigma(Y_{-m+1},\ldots,Y_0)$ be the $\sigma$-algebra generated by the internal state, and $b^{(m)}(s) := P(S_0 = s|\mathcal{F}_m)$. Then, $V(z_0) =  \mathbb E[\sum_{t=0}^\infty\gamma^tr(S_t)|\mathcal{F}_m]$. Since $V_s^\star = V_0^\star + (V_1^\star-V_0^\star)\mathbf{1}_{\{s=1\}}$ and $V(z_0) = V_0^\star + (V_1^\star-V_0^\star)b^{(m)}(1)$, we obtain $$\mathbb{E}[(V_{s_0}^\star-V(z_0))^2] = (V_1^\star-V_0^\star)^2\,\mathbb E\!\left[(\mathbf{1}_{\{s_0=1\}}-b^{(m)}(1))^2\right].$$
