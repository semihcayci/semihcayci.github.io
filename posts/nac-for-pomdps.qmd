...
To see how $m$-step TD learning with internal state $Z_t$ mitigates perceptual aliasing, let us focus on the limit point of the algorithm. To that end, let $\bar{r}_m(z) := \mathbb E[\sum_{k=0}^{m-1}r(S_k)|Z_0=z], (K_mv)(z):=\mathbb{E}[v(Z_m)|Z_0=z]$, and $(T_mv)(z) = \bar{r}_m(z) + \gamma^m(K_mv)(z)$. Then, it is straightforward to show that $T_m$ is a contractive operator with modulus $\gamma^m$, and admits a unique fixed point $$V(z) = \mathbb E[\sum_{k=0}^\infty r(S_k)|Z_0=z].$$ Alternatively, one can write $V = \sum_{k=0}^\infty (\gamma^mK_m)^k\bar{r}_m$ for any $m\in\mathbb N$. Let $\xi$ be the invariant distribution of $\{Z_k:k\in\mathbb N\}$. In the specific case of $Z_t=(Y_{t-m+1},\ldots,Y_t)$, we observe that $P(Z_t=z_t)=\sum_{s_{t-m+1:t}}\prod_{k=0}^{m-1}\Phi(y_{t-k}|s_{t-k})\mu(s_{t-k})$, where $\mu$ is the stationary distribution of $\{S_k:k\in\mathbb N\}$. Assume that the Gram matrix $\mathbb{E}_{Z_t\sim \xi}[\psi(Z_t)\otimes\psi(Z_t)]\succ 0$. Let $\pi$ be the ortogonal projection onto $\mathcal V := \{z\mapsto \langle \psi(z),w\rangle: w \in \mathbb R^d$ with respect to $L^2(\xi)$. Then, $m$-step TD learning with the internal state $Z_t$ converges to the unique fixed point $v_m$ of the equation $$v = \pi\{\bar{r}_m+\gamma^m K_m v\}.$$ Using this, we can show that $$\|v_m - \pi V_m\|_{L^2(\xi)} \leq \frac{\gamma^m}{1+\gamma^m}\|V_m-\pi V_m\|_{L^2(\xi)}.$$ As such, it is possible to learn $V(z_0)$ via $m$-step TD learning with an additional error term $\mathcal{O}(\gamma^m)$.
