...
with:
\begin{itemize}
\item latent state $S_k\in\mathcal S$,
\item action $A_k\in\mathcal A$,
\item observation $Y_k\in\mathcal Y$,
\item transition kernel $P(s' \mid s,a)$,
\item observation kernel $\Phi(y \mid s')$,
\item reward $r(s,a)\in[0,1]$ (w.l.o.g.),
...
:::
It is very easy to construct an example which shows that (memoryless) TD learning cannot learn the value functions in a process with memory [@singh1994learning]. Here, we want to compute $V(z_0):=\mathbb E[\sum_{k=0}^\infty \gamma^k r(S_k)|Z_0=z_0]$ for an internal state $Z_t$ (e.g., $Z_t = (Y_{t-m+1},\ldots,Y_t)$), where $(S_k, r(S_k), Y_k)$ is a hidden Markov reward process.
...
Let $\bar{r}_m(z) := \mathbb E[\sum_{k=0}^{m-1}r(S_k)|Z_0=z], (K_mv)(z):=\mathbb{E}[v(Z_m)|Z_0=z]$, and $(T_mv)(z) = \bar{r}_m(z) + \gamma^m(K_mv)(z)$. Then, it is straightforward to show that $T_m$ is a contractive operator with modulus $\gamma^m$, and admits a unique fixed point $V(z) = \mathbb E[\sum_{k=0}^\infty r(S_k)|Z_0=z]$. Alternatively, you can write $V = \sum_{k=0}^\infty (\gamma^mK_m)^k\bar{r}_m$ for any $m\in\mathbb N$. Let $\xi$ be the invariant distribution of $\{Z_k:k\in\mathbb N\}$. Assume that the Gram matrix $\mathbb{E}_{Z_t\sim \xi}[\psi(Z_t)\otimes\psi(Z_t)]\succ 0$. Let $\pi$ be the ortogonal projection onto $\mathcal V := \{z\mapsto \langle \psi(z),w\rangle: w in \mathbb R^d$ with respect to $L^2(\xi)$. Then, $m$-step TD learning with memory converges to the fixed point of $v_m = \pi\{\bar{r}_m+\gamma^m K_m v_m\}$. Hence, we can show that $$\|v_m - \pi V_m\|_{L^2(\xi)} \leq \frac{\gamma^m}{1+\gamma^m}\|V_m-\pi V_m\|_{L^2(\xi)}.$$ As such, it is possible to learn $V(z_0)$ via $m$-step TD learning with an additional error term $\mathcal{O}(\gamma^m)$.
